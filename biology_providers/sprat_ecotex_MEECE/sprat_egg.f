...
c     ---------------------------------------------------
c     Based on generic_bioenergetics/released_egg_stage.f 
c     MEECE specific ecotox parameterization for sprat eggs
c
...
c     ==============================================================
      ...
      real,parameter :: K2  = 50       ! yg/l ecotox parameterization from Tanja St John
      real,parameter :: A0  = 0.231794 ! ecotox parameterization from Tanja St John
      real,parameter :: A1  = 0.286788 ! ecotox parameterization from Tanja St John
      real,parameter :: A2  = 5.68092  ! ecotox parameterization from Tanja St John
...
c     --------------------------------------------------------------
      ...
      call interpolate_copper(xyz,c u, istat)  ! unit = kg/m3
      cu = cu*1.0e-6                           ! convert to unit = yg/l

      dcomp_dt = devel_prefac * temp**devel_expo ! inlined from: biology_providers/sprat/sprat_egg.f

      state%completion = state%completion + dcomp_dt*dt  ! OK for dt<0
c
c     --- set mortality_rate + die ---
c        
      S0             = A0*(1 - exp(-A1*(temp-A2)))  ! from Tanja St John experiments
      surv_prob      = S0 * exp(- cu / K2)  ! from Tanja St John experiments
      mortality_rate = -dcomp_dt*log(surv_prob) ! convert to instataneous mortality rate
      die            = .false.             ! just decrease survival
c
