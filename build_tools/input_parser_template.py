...
#
# TODO: commit template + test -> trunk
#
# Generate f90 code: 
#    input_parser_template.py > input_parser.f
# test code (using file inputexample):
#    ifort input_parser.f input_parser_test.f ; a.out
...
read_data_template = """
      ...
      subroutine read_control_data_%(tag)s(filehandler,
...
c.....loop over lines in control file
      ...
      do iline = istart, size(filehandler%(fsep)srawdata)
...
c........drop leading/trailing blanks
         if (trim(thisname) == trim(adjustl(name))) then
             ...
             read(cbuf, %(readfmt)s) value   ! here goes readfmt
...
c.....maybe a success flag should be passed 
     ...
     &           " not in "//filehandler%(fsep)sfilename
      stop
      end subroutine read_control_data_%(tag)s

"""
# ------------------------------------------------------------------------------------      
#                         Actual values for the template
#
#      notes: - use readfmt == '(A)' for strings, as backslash are considered
#               as separator token bby e.g. ifort
# ------------------------------------------------------------------------------------
#          tag        decl                              readfmt
# ------------------------------------------------------------------------------------
cases = (("char",    "character(len=*)    :: value",     "'(A)'"),
         ("int",     "integer             :: value",     "*"),
         ("real",    "real                :: value",     "*"),
         ("real8",   "real(kind=8)        :: value",     "*"),
         ("log",     "logical             :: value",     "*"),
         ("intvec",  "integer             :: value(:)",  "*"),
         ("realvec", "real                :: value(:)",  "*"),
         ("real8vec","real(kind=8)        :: value(:)",  "*"))

# ----------------------------
interface = "      interface read_control_data\n"
for (tag,decl,readfmt) in cases:
    interface = interface + ("        module procedure read_control_data_%s\n" % tag)
...
for (tag,decl,readfmt) in cases:
    this = {"tag" :    tag,
            ...
            "readfmt": readfmt,
            "fsep":    "%"}
    read_control_subs = read_control_subs + (read_data_template % this)
