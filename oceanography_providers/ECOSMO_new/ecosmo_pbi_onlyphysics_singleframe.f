...
c
c     Test version PBI for ECOSMO data output (in 2011 binary format)
c     based on full PBI ecosmo_pbi.f developed for MEECE. This version only loads physics and 
c     only loads a single data frame (irrespective of actual time) ASC@21Jun2016
c     The input file is same binary ECOSMO format, but only containing a single time frame 
c     This version does not have a hydroDB reference - all files must be in run directory
c     (avoid path separator non-portability)
c
...
c     ------------------------------------------
      ...
      real                             :: rdum
      character*999                    :: hdbfile
c     ------------------------------------------
      if (present(time)) call set_master_clock(time)
      write(*,*) trim(get_pbi_version()) 

      write(*,*) "init_physical_fields: Frame-locked ECOSMO PBI "
...
c    
     ...
     +                          "vertical_diffusivity",rdum)
      ...
      vdiffus = rdum
c
c     ----- load single fixed physics frame
c      
      write(*,*) "Frame-locked ECOSMO PBI - loading fixed physics"
      call read_control_data(simulation_file, "fixed_hydro_file", 
     +                       hdbfile)

      call open_data_files(hdbfile)
      call load_data_frames()
...
c.....read internal ECOSMO dimensions      
 ...
 234  format("read_grid_desc: ECOSMO dimension ",a," = "i8)
      ...
      call init_mesh_grid(init_biogeochem = .false.)  ! incl allocation of 3D arrays
...
c     ------------------------------------------  
c     Just update clock, no hydrography, as this 
c     version runs with fixed hydrography
c     ------------------------------------------
      type(clock), intent(in),optional :: time
...
c     ------------------------------------------
      ...
      subroutine open_data_files(fname)
c     ------------------------------------------
c     Open binary file
c     ------------------------------------------
      character*(*)    :: fname ! full path reference
      integer          :: iostat
c     ------------------------------------------
...
c
      write(*,*) "open_data_files: opened file", trim(adjustl(fname))

      end subroutine open_data_files

 
      
      subroutine load_data_frames()
c     ------------------------------------------
c     Load fixed frame one, only physics
c     
...
c     read(bio_file)  Tc   ! do not read biogeochemistry
...
c
c     ---- skip update of biogeochemistry  
 
      write(*,233) mjarcc,lmoncc,ndaycc
 233  format("load_data_frames: frame time labels = ",3i3,")")
