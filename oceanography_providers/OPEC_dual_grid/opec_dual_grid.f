...
c     ------------------------------------------------------------
c     OPEC cmod+ERGOM dual grid pbi (coarse+fine grid at same time)
...
c    
c     TODO: currently "cmod_data_set" = "dual" expands to coarse+fine
c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c     test_undef: safe value > 0 to detect undef values by real comparisons, like: if (myval < unsetlim) < actions >
     ...
     +             interpolate_currents_cc => interpolate_currents ! use face-centered local
      ...
      use embedded_hydrography,    ! postpend imported name space with _f to avoid clash with mesh_grid
     +     interpolate_currents_f   => interpolate_currents,
     +     interpolate_wdepth_f     => interpolate_wdepth, 
     +     is_wet_f                 => is_wet,
     +     horizontal_range_check_f => horizontal_range_check,
     +     nx_f                     => nx,
     +     ny_f                     => ny,
     +     nz_f                     => nz,
     +     u_f                      => u,
     ...
     +     dslm_f                   => dslm,
     +     ccdepth_f                => ccdepth,
     +     acc_width_f              => acc_width,
     +     wdepth_f                 => wdepth,
     +     bottom_layer_f           => bottom_layer,
     +     wetmask_f                => wetmask
...
c     ------ data frame handler ------
      ...
      integer            :: data_set_handler_f ! cmod data map for embedded set
      logical            :: embedded_hydro     ! whether or not embedded hydrography is applied
...
c     --- 3D grids ---
      ...
      real,allocatable,target :: ccdepth0_f(:,:,:)   ! embedded grid: ref cc depth water below surface [m] (dslm=0)
      real,allocatable,target :: acc_width0_f(:,:,:) ! embedded grid: accu water above this undisturbed layer [m] dim=nz+1
      
c     --- 2D grids ---
      
      real,allocatable :: wdepth0(:,:)      ! reference depth at cell-center, positive  [m] - dry points negative
      real,allocatable :: wdepth0_f(:,:)    ! embedded grid: ref depth at cell-center, positive  [m] - dry points negative
...
c     ---------------------------------------------------
      character*99         :: data_set_id, data_set_id_f
      integer              :: ix,iy,iz,idx1d,j,ibot
      
      real                 :: lambda1, dlambda, phi1, dphi ! LOCAL DUMMIES
      integer              :: kx,ky,kz                     ! LOCAL DUMMIES
      real                 :: l1,p1,dl,dp ! LOCAL DUMMIES
c     ---------------------------------------------------
c
...
c   
     ...
     +     data_set_id)
      if (trim(adjustl(data_set_id)) == "dual") then
         write(*,*) "Using dual data sets (coarse+fine)"
         data_set_id    = "coarse"
         data_set_id_f  = "fine"
         embedded_hydro = .true.
      else ! single grid mode
         write(*,*) "Using single data set:", trim(adjustl(data_set_id))
         data_set_id_f  = "NA"   
         embedded_hydro = .false.
      endif
...
c     get_grid_descriptors is defined in module read_cmod
 ...
 233  format("read_grid_desc: phi1    = ",f12.7,
     +                      " dphi    = ",f12.7," degrees")  
      ...
      if (embedded_hydro) then
         call get_grid_descriptors(data_set_id_f, data_set_handler_f,     
     +                             l1,dl,p1,dp, kx, ky, kz) 

         call init_embedded_hydrography(kx,ky,kz,l1,p1,dl,dp) ! defines nx_f, ny_f, nz_f 
         
         write(*,*) "read_grid_desc: embedded grid"
         write(*,231) nx_f, ny_f, nz_f 
         write(*,232) l1, dl
         write(*,233) p1, dp
...
c     --- 3D grids ---
      ...
      if (embedded_hydro) allocate( ccdepth0_f(nx_f,ny_f,nz_f)     ) 
      if (embedded_hydro) allocate( acc_width0_f(nx_f,ny_f,nz_f+1) )  
      
c     --- 2D grids ---
     
      allocate( wdepth0(nx,ny)       )
      if (embedded_hydro) allocate( wdepth0_f(nx_f,ny_f) ) 
...
c     ------------------------------------------------------------------------------
c     set auxillary grid arrays: ccdepth0_f, acc_width0_f, bottom_layer_f, wetmask_f, wdepth0_f
c     for embedded grid, if active
c     ------------------------------------------------------------------------------   
      if (embedded_hydro) then
         
          wdepth0_f      = -1.0       ! default dry
          bottom_layer_f =  0     ! default dry
c          
c         do not assign a value to ccdepth0/acc_width0 to be able to spot unknown dry point violations
c
          acc_width0_f(:,:,1)  = 0. ! sea surface (DSLM=0) 
          acc_width0_f(:,:,2:) = 1.e6 ! acc_width0 must be inceasing
          do ix=1,nx_f
             do iy=1,ny_f
                if (m1_cmod(data_set_handler_f)%
     +                      p(ny_f+1-iy, ix, 1)>0) then ! wet/dry longitunal point test  
c             
c                  Vertical loop for wet points. At loop exit iz will point to first dry layer
c                  F90 std condition for full do-loop termination is iz=nz+1
c            
                   ...
                   do iz=1,nz_f 
                      idx1d = m1_cmod(data_set_handler_f)%
     +                                p(ny_f+1-iy,ix,iz)
                      if (idx1d > 0) then ! wet/dry signal for cmod   
                         ccdepth0_f(ix,iy,iz) = 
     +                        hz_cmod(data_set_handler_f)%p(idx1d)
                         acc_width0_f(ix,iy,iz+1) =
     +                        acc_width0_f(ix,iy,iz) + 
     +                        cellw_cmod(data_set_handler_f)%p(idx1d)
                      else
                         exit 
                      endif   
                   enddo        ! iz loop
                   ...
                   ibot                = iz-1 ! last wet layer
                   bottom_layer_f(ix,iy) = ibot
                   wdepth0_f(ix,iy) = acc_width0_f(ix,iy,1+ibot)                                  
c        
                endif           ! wet/dry longitunal point
c                
             enddo              ! iy loop
...
c
      ...
      if (embedded_hydro) then
c         
         call get_u(u_f, data_set_handler_f)
         call get_v(v_f, data_set_handler_f)
         call get_w(w_f, data_set_handler_f)
         call get_z(dslm_f, data_set_handler_f)
c         
         where (u_f > unsetlim) 
            u_f=0
         end where
...
c            Jun 16, 2011: capture rare cases where dz creates negative depths
      ...
      if (embedded_hydro) then
        do ix=1,nx_f
           do iy=1,ny_f
              if (wetmask_f(ix,iy)==0) cycle ! nothing for dry points
              dz                    = dslm_f(ix,iy)
              wdepth_f(ix,iy)       = max(0.0, wdepth0_f(ix,iy) + dz) ! NB: flipped sign
              acc_width_f(ix,iy,2:) = acc_width0_f(ix,iy,2:) + dz ! NB: flipped sign
              ccdepth_f(ix,iy,1)    = ccdepth0_f(ix,iy,1)    + dz*0.5 ! NB: flipped sign
              ccdepth_f(ix,iy,2:)   = ccdepth0_f(ix,iy,2:)   + dz ! NB: flipped sign
           enddo
        enddo
      endif
...
c     ------------------------------------------ 
      ...
      real                 :: x,y,z, sx,sy,sz,zf
c     ------------------------------------------
c     First check whether xyz is inside the embedded grid and wet
c     If so, prefer this interpolation, otherwise continue to the
c     base grid. Rely on left-to-right evaluation below
      
      if ((embedded_hydro) .and.
     +    horizontal_range_check_f(xyz) .and.
     +    is_wet_f(xyz)) then
         call interpolate_currents_f(xyz, uvw, status)
         return ! with whatever value assigned in interpolate_currents_f
      endif
