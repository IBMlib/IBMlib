...
c     Inline code for coast_line_intersection provision in relation to a grid
c
c     Different variant implementations should be soft-linked to
c     coastline_intersection_for_grid.f on build time, as regular_lonlat_grid.f
c     includes content of file coastline_intersection_for_grid.f
c      
...
c     adapted from horizontal_representation_scan_staywet.f: ASC 12 Dec, 2019
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      
      subroutine coastline_intersection_for_grid(this,
     +     geo1, geo2,
     +     cross_coast,
     +     georef, geohit)      
c     ------------------------------------------ 
...
c     ---------------------------------------------------- 
      call get_horiz_ncc_index(this,geo1,ix1,iy1)  ! resolve start point cell
      call get_horiz_ncc_index(this,geo2,ix2,iy2)  ! resolve end point cell
...
c
      if (this%wetmask(ix1,iy1)==0) then
         write(*,347) geo1(1:2)        
...
c
      ...
      if ((ix1==ix2).and.(iy1==iy2)) then 
         ...
         iymin   = min(iy1,iy2)
         iymax   = max(iy1,iy2)
         all_wet = all(this%wetmask(ixmin:ixmax,iymin:iymax) > 0) ! indices in correct order
      endif
...
c     The continued analysis beyond this point is in grid coordinates
     ...
     +              "intercell step scan intermediate cells: begin"
      ...
      call get_horiz_grid_coordinates(this,geo1,xy1)
      call get_horiz_grid_coordinates(this,geo2,xy2)
c
...
c     Infinitesimal steps are into side/corner neighbor cells of starting cell
      ...
      if ((this%wetmask(ix2,iy2) == 0).and.  ! step into dry box
...
c
      do ix = ix1,ix2       ! start in geo1 cell
         do iy = iy1,iy2    ! start in geo1 cell
            ...
            if (this%wetmask(ix,iy)==0) then ! only analyze dry cells
...
c     conclude no coastal crossings 1->2 
      ...
      if ((.not.cross_coast).and.(this%wetmask(ix2,iy2)==0)) then
...
c     |direct| == |reflect|        
      ...
      call get_horiz_geo_coordinates(this,xyref,georef) ! return to geo coordinates (only modifies position 1:2) 
      call get_horiz_geo_coordinates(this,xyhit,geohit) ! return to geo coordinates (only modifies position 1:2) 
c
...
c     -----------------------------------------------
      ...
      logical function point_on_land(maybewet, wetpt,
     +     assured_wetpt)
c     --------------------------------------------------------------
...
c     ------------------------------------------------------------      
      point_on_land = is_land_for_grid(this, maybewet) ! .true. : needs nudging
...
c   
      ...
      do while (keep_going)
         ...
         keep_going          = is_land_for_grid(this, assured_wetpt)
         if (istep > max_steps) then
            write(*,*) "point_on_land: max_steps exceeded"
            write(*,*) "maybewet         =", maybewet
            write(*,*) "wetpt            =", wetpt
            write(*,*) "is_land(maybewet)=",
     +           is_land_for_grid(this, maybewet)
            write(*,*) "is_land(wetpt)   =",
     +           is_land_for_grid(this, wetpt)
...
c     -------------------------------------------
      if (cross_coast) then 
         ...
         write(*,*) "is_land(geo1)   = ", is_land_for_grid(this,geo1)
         write(*,*) "is_land(geo2)   = ", is_land_for_grid(this,geo2)
         write(*,*) "is_land(georef) = ", is_land_for_grid(this,georef)
         write(*,*) "is_land(geohit) = ", is_land_for_grid(this,geohit)
      else
 ...
 623  format("summary of coast_line_intersection: ",
     +        2f8.3, " ->", 2f8.3, " : no coast crossed")              
      ...
      end subroutine coastline_intersection_for_grid ! embedding subroutine
...
c     (no reference to grid / module inclusion necessary)      
c     ------------------------------------------
