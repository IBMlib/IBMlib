#!/usr/bin/env python
#####################################################################################
#  Baseline 3 GF run (smagorinsky diffusivity)
#
...
#  make clean; tar cvfz ../tarball.tgz *
#  make ibmrun; link
#  -------------------- HPC voodoo (LSF) --------------------
#  ssh asch@login.gbar.dtu.dk
#  scp ../tarball.tgz   asch@login.gbar.dtu.dk:CLAIM/WP1/IBMlib
#  scp run_GF_baltic_baseline.py asch@login.gbar.dtu.dk:CLAIM/WP1/IBMlib
...
"""
stations = {
    "Oder"    : (14.60417, 53.55416),
    "Nemunas" : (21.26250, 55.32917),
    "Narva"   : (28.04583, 59.47083),
    "Wisla"   : (18.93750, 54.31250),
    "Luga"    : (28.27083, 59.65417),
    "Daugava" : (23.85417, 56.97083)
}



(srate, ktrans) =  (0.003, 0.07)
            
for stat in stations.keys():
    jobtag      = "%s_%d-%d_srate_%5.3f_hdiff=smag_ktrans_%4.2f" % (stat, simintval[0], simintval[1], srate, ktrans)
    simfilename = "simpar_" + jobtag
    srcfilename = "src_%s.lonlat" % stat
    job_params  = {"hydroDBroot" : hydroDBroot,
                  ...
                  "npart"       : npart,
                  "outputfile"  : "GF_" + jobtag + ".nc",
                  "sources"     : srcfilename}

    #
    #  -------------------- generate GF source file: src.lonlat --------------------
    #
    f = open(srcfilename, "w")
    f.write("#    lon           Lat      discharge\n")
    f.write("%f %f 1.000" % stations[stat])  # unit influx
    f.close()        
    #
    ...
    os.system('bsub -n 1 -W 24:00 -R "rusage[mem=16GB]" -J GF_%s -o %s.o < %s' % (stat, jobfile_name, jobfile_name))  # system specific command
    #
