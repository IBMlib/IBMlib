...
c      public :: interpolate_dslm    ! currently unused
...
c     ---- currently no horizontal turbulent diffusivity: set to zero ----  
...
c     ------------------------------------------ 
      ...
      integer          :: status
c     ------------------------------------------ 
      call interpolate_wdepth(xy, wdepth, status) 
      ...
      elseif (status==0) then
         ...
         stop "is_land: unhandled return condition"
      endif  
c     ------------------------------------------
      end function
...
c     ------------------------------------------
c     Detect wheter cell association of xy
...
c
      anycross = ((.not.xyz1_is_land).and.xyz2_is_land).or.
     +           ((.not.xyz2_is_land).and.xyz1_is_land)
...
c 
      if (anycross) then
c        --- define dry cell corners (c00, c01, c11, c10) --- 
         c00(1) = xyz2(1) - 0.5*dlambda
         c01(1) = xyz2(1) - 0.5*dlambda
         c10(1) = xyz2(1) + 0.5*dlambda
         c11(1) = xyz2(1) + 0.5*dlambda

         c00(2) = xyz2(2) - 0.5*dphi
         c01(2) = xyz2(2) + 0.5*dphi
         c10(2) = xyz2(2) - 0.5*dphi
         c11(2) = xyz2(2) + 0.5*dphi
c
...
c     --- there should be a solution, if we did end in this if clause
     ...
     +       stop "coast_line_intersection: unexpected condition"
         ...
         xyzhit = xyz1   +     s*direct 
         xyzref = xyzhit + (1-s)*reflect
